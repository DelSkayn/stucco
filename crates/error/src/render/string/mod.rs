use crate::{
    AnnotationKind, Diagnostic, Level, Snippet,
    render::string::char_buffer::{Color, Styling},
};
...
pub fn render_char_buffer(g: &Diagnostic<'_>) -> CharBuffer {
    let primary_group = g
        .groups
        .first()
        .expect("Diagnostic must atleast have a single group");
...
fn render_element(buf: &mut CharBuffer, elem: &Snippet, line_color: Color) {
    ...
    let largest_offset = elem
        .annotations
        .iter()
        .map(|x| x.span.byte_range().end)
        .max()
        .unwrap_or_default();
    ...
    let Some(prime) = elem
        .annotations
        .iter()
        .find(|e| e.kind == AnnotationKind::Primary)
    else {
        return;
    };

    let prime_loc = Loc::from_offset_in_str(source, prime.span.byte_range().start);

    let mut anns = elem
        .annotations
        .iter()
        .map(|x| {
            let byte_range = x.span.byte_range();
            (
                x,
                Loc::from_offset_in_str(source, byte_range.start),
                Loc::from_offset_in_str(source, byte_range.end),
            )
        })
        .collect::<Vec<_>>();

    anns.sort_unstable_by(|a, b| {
        a.1.line
            .cmp(&b.1.line)
